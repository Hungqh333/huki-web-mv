...
import { PARSER_MODEL, runParser, type ParserOutcome } from './parserCore';
 ...
 * không tự thử lại (attempts: 1) để luôn trả kết quả trước giới hạn đó — thất bại
 * (kể cả hết hạn mức gói miễn phí, lỗi 429) thì người dùng vẫn có bảng trống và
 * nút "Đọc lại".
 *
...
export async function parseDescription(rawText: string): Promise<ParserOutcome> {
  ...
  return runParser(client, rawText, { model: process.env.GEMINI_MODEL || PARSER_MODEL });
}
